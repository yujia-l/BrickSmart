"""Semantic-preservation helpers for confirmed model contracts.

This module checks that source segment identities, labels, and lineage survive
contract loading and runtime preprocessing.
"""
...
def segment_id_from_row(row: Mapping[str, Any]) -> int | None:
    """Return segment id from row.
...
def confirmation_status(row: Mapping[str, Any], default: str = "unresolved") -> str:
    """Return the confirmation status value.
...
def _group_source_ids(
    ...
    :param confirmation_rows: The confirmation rows value.
    :type confirmation_rows: list[Mapping[str, Any]]
    :returns: The result produced by the function.
...
def build_semantic_target_preservation_report(
    *,
    ...
    """Build semantic target preservation report.
    
    :param context: Runtime context for the operation.
    :type context: Mapping[str, Any]
    :param confirmation_rows: The confirmation rows value.
    :type confirmation_rows: list[Mapping[str, Any]]
    :param raw_counts: The raw counts value.
    :type raw_counts: Mapping[int | str, int]
    :param clean_counts: The clean counts value.
    :type clean_counts: Mapping[int | str, int]
    ...
    authoritative_ids = {
        ...
        if confirmation_status(row) in AUTHORITATIVE_CONFIRMATION_STATUSES
        and (sid := segment_id_from_row(row)) is not None
    }
