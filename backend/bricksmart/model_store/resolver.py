...
This module resolves explicit URI-style references and local filesystem paths,
including model-store and checksum-backed references.
"""
...
_EXPLICIT_URI_DELIMITER = "://"


def _is_explicit_uri(value: str) -> bool:
    """Return True only for references that intentionally use URI syntax."""
    return _EXPLICIT_URI_DELIMITER in str(value or "")


def _path_from_file_uri(parsed: ParseResult) -> Path:
    """Convert a parsed ``file://`` URI to a local ``Path``."""
    path_text = unquote(parsed.path or "")
    host = unquote(parsed.netloc or "")
    if host and host.lower() != "localhost":
        return Path(f"//{host}{path_text}")
    is_windows_drive_file_uri = (
        os.name == "nt"
        and path_text.startswith("/")
        and len(path_text) >= 3
        and path_text[2] == ":"
    )
    if is_windows_drive_file_uri:
...
class ModelResolver:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the ModelResolver instance.
        
        :param project_root: The project root value.
        :type project_root: str | Path
        :param store: The store value.
        :type store: LocalModelStore | None
        :param allow_remote: Whether to allow remote.
        :type allow_remote: bool | None
        :param remote_host_allowlist: The remote host allowlist value.
        :type remote_host_allowlist: set[str] | None
        """
        ...
        self.remote_host_allowlist = (
            remote_host_allowlist
            if remote_host_allowlist is not None
    ...
    def resolve(
        ...
        *,
        ...
        :type source: ModelSourceSpec | str | Mapping[str, Any]
        :param context_path: Path to the context file.
        :type context_path: str | Path | None
        :param default_model_id: Identifier for the default model.
