"""Command-line inspection helpers for the block catalog.

The catalog CLI loads the authoritative CSV catalog and reports normalized block
definitions, schema status, and catalog-derived planning metadata.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Inspect the authoritative BrickSmart block_definitions.csv catalog"
    )
    parser.add_argument(
        "--catalog",
        type=Path,
        default=Path("block_catalog/block_definitions.csv"),
    )
