"""Deterministic voxelization helpers for model geometry.

This module converts source meshes and segments into grid-aligned voxels while
preserving source-segment lineage.
"""
...
def _component_filter(voxels: set[GridCoord], minimum_size: int) -> set[GridCoord]:
    """Return the component filter value.
    
    :param voxels: The voxels value.
    :type voxels: set[GridCoord]
    :param minimum_size: The minimum size value.
...
def voxelize_segmented_model(
    ...
    *,
    ...
    """Return the voxelize segmented model value.
    
    :param model: Model object or identifier used by the operation.
    :type model: LoadedObjModel
