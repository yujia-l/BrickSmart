...
from bricksmart.row_column.artifacts import (
    json_safe_value,
...
from bricksmart.row_column.geometry import (
    FaceType,
...
from bricksmart.row_column.runtime_paths import (
    read_json,
    resolve_input_with_project_fallback,
    resolve_path,
)
from bricksmart.row_column.validation import (
    catalog_run_audit,
...
from bricksmart.row_column.visualization import (
...
def configured_path_base(paths_config, context_dir):
    """Return the configured path base.
    
    :param paths_config: The paths config value.
    :param context_dir: Directory for context artifacts.
...
def resolve_catalog_with_sibling_fallback(value, path_base):
    ...
    Resolution order:
      ...
      3. a unique block_definitions*.csv file in that sibling directory.

    If several fallback catalog files exist, fail rather than guessing.
    """
    ...
    configured_basename = (
        ...
        else "block_definitions.csv"
    )
    explicit_candidates = [
        sibling_catalog_dir / configured_basename,
        sibling_catalog_dir / "block_definitions.csv",
    ]
    ...
    if len(fallback_matches) > 1:
        raise FileNotFoundError(
            ...
            "block_definitions*.csv files exist in the sibling catalog "
            "directory. Set paths.catalog_csv to the exact filename. "
...
def runtime_path_layout_summary(
    *,
    ...
    """Return the runtime path layout summary.
    
    :param path_base: The path base value.
    :param path_base_mode: The path base mode value.
    :param context_dir: Directory for context artifacts.
    :param source_model: The source model value.
...
CATALOG_CSV_CONFIG_PATH = (
    resolve_catalog_with_sibling_fallback(
        PATHS.get(
            "catalog_csv",
            TASK_CONTEXT.get("catalog_csv"),
        ),
        PATH_BASE_DIR,
    )
)
...
INPUT_DIAGNOSTICS = {
    ...
    "catalog": str(CATALOG_CSV_CONFIG_PATH),
    "catalog_exists": bool(CATALOG_CSV_CONFIG_PATH and CATALOG_CSV_CONFIG_PATH.is_file()),
    ...
    "path_layout": runtime_path_layout_summary(
        ...
        catalog=CATALOG_CSV_CONFIG_PATH,
    ),
}
...
if CATALOG_CSV_CONFIG_PATH is None or not CATALOG_CSV_CONFIG_PATH.is_file():
    sibling_catalog_dir = (
        RUNTIME_WORKING_DIR.parent
        / "block_catalog"
    ).resolve()
    raise FileNotFoundError(
        "The task context paths.catalog_csv is missing or invalid. "
        f"Resolved path: {CATALOG_CSV_CONFIG_PATH}. "
        "The runtime expects the shared catalog at: "
        f"{sibling_catalog_dir / 'block_definitions.csv'}"
    )
...
if log_enabled(
    ...
    print("Catalog:", CATALOG_CSV_CONFIG_PATH)
...
MODEL_COMPATIBILITY_PREFLIGHT = {
    ...
    "notes": [
        ...
        "The catalog is resolved from the shared CSV catalog; inventory remains a separate run constraint.",
    ],
}
...
def obj_to_voxel_with_segments(file_path, voxel_size=16, samples_per_triangle=30):
    """Convert OBJ data to to voxel with segments.
    
    :param file_path: Path to the file file.
    :param voxel_size: The voxel size value.
    :param samples_per_triangle: The samples per triangle value.
    :returns: The result produced by the function.
    """
...
def adjacency_to_dict(adjacency_pairs):
    """Convert adjacency to to dict.
    
    :param adjacency_pairs: The adjacency pairs value.
    :returns: The result produced by the function.
    """
...
def generate_segment_colors(voxel_segment, adjacency_dict):
    ...
    :param adjacency_dict: The adjacency dict value.
...
def create_segment_legend(
    ...
    :param segment_names: The segment names value.
    :param swatch_size: The swatch size value.
...
def _apply_segment_confirmation(record, default_source='teacher_frontend'):
    """Apply segment confirmation.
    
    :param record: The record value.
    :param default_source: The default source value.
    """
...
def _normalized_obj_semantic_name(value):
    """Return normalized obj semantic name.
...
def _source_name_for_segment(segment_id):
    """Return source name for segment.
...
def resolve_block_catalog_path():
    """Resolve block catalog path.
    
    :returns: The computed result.
    """
    return CATALOG_CSV_CONFIG_PATH
...
def face_grid_shape(world_size, face):
    """Return face grid shape.
    
    :param world_size: The world size value.
...
def make_catalog_face_template(
    ...
    """Create catalog face template.
    
    :param world_size: The world size value.
    :param male_faces: The male faces value.
    :param female_faces: The female faces value.
    :returns: The generated result.
    """
    template = {}
...
CATALOG_CSV_PATH = resolve_block_catalog_path()
_CATALOG = load_block_catalog(CATALOG_CSV_PATH)
BLOCK_CATALOG_RECORDS = [
    dict(definition.raw_metadata)
...
def catalog_priority_missing(
    ...
    """Return whether catalog priority missing.
...
def catalog_priority_as_float(
    ...
    """Return catalog priority as float.
    
    :param block_family: The block family value.
    :param value: Value used by the operation.
    :returns: The result produced by the function.
    """
    if catalog_priority_missing(
        ...
        raise ValueError(
            ...
            f"{CATALOG_CSV_PATH}. Replace the local catalog file with "
            "the block_definitions.csv supplied in the authoritative catalog."
        )
    try:
        return float(
            value
...
if not invalid_catalog_priority_rows.empty:
    ...
    raise ValueError(
        ...
        f"{CATALOG_CSV_PATH} with the corrected "
        "block_definitions.csv supplied in the authoritative catalog."
    )
...
if log_enabled(
    ...
    print(f"Loaded block catalog: {CATALOG_CSV_PATH}")
...
def inventory_requirements_from_results(results):
    """Return inventory requirements from results.
...
def inventory_commit_results_atomic(results, scope):
    """Return inventory commit results atomic.
...
class BlockInstance:
    def __init__(
        ...
        """Initialize the BlockInstance instance.
        ...
        :param connector_type: The connector type value.
        :param segment_a: The segment a value.
        :param segment_b: The segment b value.
        :param block_family: The block family value.
        """
...
def voxel_to_2x2_columns(voxel_matrix):
    """Convert voxel data to to 2x2 columns.
...
def male_face_for_rotation(rotation, block_size=None):
    """Return male face for rotation.
    
    :param rotation: The rotation value.
    :param block_size: The block size value.
    :returns: The result produced by the function.
    """
...
def larger_block_tie_break_key(sequence):
    """Return larger block tie break key.
...
def sequence_options_for_column(height, config):
    """Return sequence options for column.
...
def fixed_column_minimum_block_count(columns):
    """Return fixed column minimum block count.
...
def count_aligned_seams(
    ...
    :param column_key: The column key value.
    :param sequence: The sequence value.
    :param current_assignment: The current assignment value.
    :param previous_row_assignment: The previous row assignment value.
    :returns: The result produced by the function.
    """
...
def build_row_blocks_from_assignment(
    ...
    """Build row blocks from assignment.
    
    :param row_columns: The row columns value.
...
def locking_reachable_ids(
    ...
    """Return locking reachable ids.
    
    :param current_ids: Identifiers for the current records.
    :param internal_lock_edges: The internal lock edges value.
    :param seed_ids: Identifiers for the seed records.
    :returns: The result produced by the function.
    """
    adjacency = {
        int(block_id): set()
...
def allow_multiple_root_components():
    """Return whether allow multiple root components.
...
def locking_component_summary(
    ...
    """Return locking component summary.
    
    :param block_ids: Identifiers for the block records.
    :param lock_edges: The lock edges value.
    :returns: The result produced by the function.
    """
    block_ids = {
        int(value)
...
def final_block_locking_graph_summary(
    ...
    """Return final block locking graph summary.
...
def planner_diagnostic_attempt_limit():
    """Return planner diagnostic attempt limit.
...
def planner_attempt_diagnostic_row(
    *,
    ...
    """Return planner attempt diagnostic row.
    
    :param segment_id: Identifier for the segment.
    :param segment_label: The segment label value.
    :param failed_row: The failed row value.
    :param row_index: The row index value.
    :param parent_state_index: The parent state index value.
    :param packing_index: The packing index value.
    :param packing: The packing value.
    :param row_blocks: The row blocks value.
    :param attempt_rank: The attempt rank value.
    :param attempt: The attempt value.
    :param is_root_row: Whether root row is enabled.
    :param is_terminal_row: Whether terminal row is enabled.
    :returns: The result produced by the function.
    """
    evaluation = (
        attempt.get(
...
def planner_attempt_contact_rows(
    ...
    """Return planner attempt contact rows.
    
    :param attempt_row: The attempt row value.
    :param attempt: The attempt value.
    :returns: The result produced by the function.
    """
    if not attempt:
...
def planner_failure_recommendation(
    ...
    """Return planner failure recommendation.
    
    :param attempt_rows: The attempt rows value.
    :returns: The result produced by the function.
    """
    reason_tokens = [
        token
...
def write_segment_planner_failure_diagnostics(
    *,
    ...
    """Write segment planner failure diagnostics.
    
    :param segment_id: Identifier for the segment.
    :param segment_label: The segment label value.
    :param failed_row: The failed row value.
    :param attempt_rows: The attempt rows value.
    :param contact_rows: The contact rows value.
    :param output_dir: Directory where generated artifacts are written.
    :returns: The result produced by the function.
    """
    output_dir = Path(
...
def planning_state_signature(state):
    """Return planning state signature.
...
def write_better_planner_outputs(
    ...
    """Compatibility wrapper for the extracted planner artifact writer."""
    return write_planner_outputs(
        planning_result,
        output_dir=output_dir,
        catalog_csv_path=CATALOG_CSV_PATH,
        structural_catalog_records=STRUCTURAL_CATALOG_RECORDS,
        task_context_json_path=TASK_CONTEXT_JSON_PATH,
        task_context_packing_priority_overrides=(
            TASK_CONTEXT_PACKING_PRIORITY_OVERRIDES
        ),
        manual_packing_priority_overrides=(
        ...
        male_face_resolver=male_face_for_rotation,
        show_output_paths=log_enabled("show_output_paths"),
    )
...
def validate_planned_instruction_steps(
    ...
    """Validate planned instruction steps.
    
    :param blocks: Block records used by the operation.
    :param instruction_steps: The instruction steps value.
    :param connector_face_requirements: The connector face requirements value.
    :returns: The result produced by the function.
    """
    block_lookup = {
        int(block.block_id): block
...
def write_step_validation_outputs(
    ...
    """Compatibility wrapper for the extracted validation artifact writer."""
    return write_step_validation_artifacts(
        validation,
        output_dir=output_dir,
        show_output_paths=log_enabled("show_output_paths"),
    )
...
def connection_type_catalog_constraint(
    ...
    """Return connection type catalog constraint.
    
    :param connection_type: The connection type value.
    :returns: The result produced by the function.
    """
    return (
        TASK_CONTEXT.get(
...
def apply_connection_type_catalog_constraint(
    ...
    """Apply connection type catalog constraint.
    
    :param connection_type: The connection type value.
    :param base_query: The base query value.
    :returns: The result produced by the function.
    """
    constraint = (
...
def llm2_extract_json_object(text):
    """Return LLM2 extract json object.
...
def llm2_catalog_capability_rows():
    """Return LLM2 catalog capability rows.
...
def llm2_catalog_allowed_values():
    """Return LLM2 catalog allowed values.
...
def llm2_validate_catalog_requirements(
    ...
    """Return LLM2 validate catalog requirements.
...
def llm2_ollama_chat_json(
    ...
    """Return LLM2 ollama chat json.
    
    :param system_prompt: The system prompt value.
    :param user_payload: The user payload value.
    :returns: The result produced by the function.
    """
...
def llm2_interface_fallback_query(
    ...
    """Return LLM2 interface fallback query.
    
    :param declared_rule: The declared rule value.
    :returns: The result produced by the function.
    """
...
def llm2_resolve_interface_decision(
    ...
    """Return LLM2 resolve interface decision.
    
    :param interface_row: The interface row value.
    :param declared_rule: The declared rule value.
    :returns: The result produced by the function.
    """
    interface_id = str(
...
def llm2_functional_fallback_query(
    ...
    """Return LLM2 functional fallback query.
...
def confirmed_exact_block_family_requirement(
    ...
    """Return confirmed exact block family requirement.
...
def validate_instructor_build_intent_contract(
    ...
    """Validate instructor build intent contract.
...
def llm2_resolve_functional_decision(
    ...
    """Return LLM2 resolve functional decision.
...
def prepare_general_catalog_record(
    ...
    either a raw catalog row or an already prepared record.
    """
    prepared = dict(
        record
    )
...
def oriented_catalog_variants(record):
    """Return oriented catalog variants.
...
def full_face_template(size, face_roles):
    """Return full face template.
    
    :param size: The size value.
    :param face_roles: The face roles value.
    :returns: The result produced by the function.
    """
    template = {}
    for face in ALL_FACES:
...
def planner_segment_grid(voxel_grid):
    """Return planner segment grid.
    
    :param voxel_grid: The voxel grid value.
    :returns: The result produced by the function.
    """
    axis_order = tuple(
        int(value)
...
def detect_segment_interfaces(segment_grid, included_segment_ids=None):
    """Detect segment interfaces.
    
    :param segment_grid: The segment grid value.
    :param included_segment_ids: Identifiers for the included segment records.
    :returns: The result produced by the function.
    """
    included = (
        None
...
def symmetry_axis_index():
    """Return symmetry axis index.
...
def configured_symmetry_center_plane(occupied_mask, axis):
    """Return the configured symmetry center plane.
    
    :param occupied_mask: The occupied mask value.
    ...
    minimum = int(coordinates[:, axis].min())
    maximum = int(coordinates[:, axis].max())
    bbox_center = (minimum + maximum + 1) / 2.0

    configured = TASK_CONTEXT.get("symmetry", {}).get("center_plane")
    ...
    for center_plane in sorted(set(candidates)):
        mirrored = mirror_mask(
            occupied_mask,
            axis,
            center_plane,
        )
        score = mask_iou(occupied_mask, mirrored)
        scored.append((
            score,
            -abs(float(center_plane) - float(bbox_center)),
            float(center_plane),
        ))
...
def physical_target_mask(
    ...
    """Return physical target mask.
    
    :param target_row: The target row value.
    :param segment_grid: The segment grid value.
    :returns: The result produced by the function.
    """
    return np.isin(
...
def run_functional_semantic_preflight(
    ...
    """Run functional semantic preflight.
    
    :param segment_table: The segment table value.
    :param provisional_targets: The provisional targets value.
    :param segment_grid: The segment grid value.
    :returns: The result produced by the function.
    """
    ...
    axis = symmetry_axis_index()
    center_plane = (
        configured_symmetry_center_plane(
            segment_grid > 0,
            axis,
        )
    )
...
def attachment_declarations():
    """Return attachment declarations.
...
class DisjointSet:
    def __init__(self, values):
        """Initialize the DisjointSet instance.
    ...
    def union(self, value_a, value_b):
        ...
        :param value_a: The value a value.
        :param value_b: The value b value.
        :returns: The result produced by the function.
        """
...
def required_assembly_interfaces(
    ...
    """Return required assembly interfaces.
    
    :param interfaces_df: DataFrame containing interfaces records.
    :param structural_segment_ids: Identifiers for the structural segment records.
    :returns: The result produced by the function.
    """
...
def boundary_contact_area(box, target_mask):
    """Compute boundary contact area.
    
    :param box: The box value.
    :param target_mask: The target mask value.
    :returns: The result produced by the function.
    """
...
def exact_2x2_footprint_mask(mask):
    """Return exact 2x2 footprint mask.
...
def candidate_origins_near_interface(
    ...
    """Return candidate origins near interface.
    
    :param size: The size value.
    :param normal_face: The normal face value.
    :param coordinate_pairs: The coordinate pairs value.
...
def generate_structural_connector_candidates(
    ...
    """Generate structural connector candidates.
    ...
    :param structural_segment_ids: Identifiers for the structural segment records.
    :param interface_catalog_queries: The interface catalog queries value.
    :returns: The generated result.
    """
    selector_name = (
        TASK_CONTEXT.get(
...
def select_nonoverlapping_candidates(
    ...
    """Select nonoverlapping candidates.
    
    :param candidates_df: DataFrame containing candidates records.
    :param group_column: The group column value.
    :param initial_reserved_coordinates: The initial reserved coordinates value.
    :returns: The computed result.
    """
...
def find_anchor_for_target(source_segment_ids, structural_ids):
    """Find anchor for target.
    
    :param source_segment_ids: Identifiers for the source segment records.
    :param structural_ids: Identifiers for the structural records.
    :returns: The computed result.
    """
    best = None
...
def target_candidate_origins(target_mask, size):
    """Return target candidate origins.
...
def is_wheel_catalog_record(
    ...
    """Return whether wheel catalog record.
...
def validate_selected_wheel_orientations(
    ...
    """Validate selected wheel orientations.
    
    :param selected_functional_df: DataFrame containing selected functional records.
    :returns: The result produced by the function.
    """
...
def functional_record_family(
    ...
    """Return functional record family.
...
def functional_variants_for_target(
    ...
    """Return functional variants for target.
...
def big_wheel_candidate_origins(
    ...
    """Return big wheel candidate origins.
...
def functional_candidate_origins(
    ...
    """Return functional candidate origins.
...
def functional_candidate_geometry_mask(
    ...
    """Return functional candidate geometry mask.
...
def functional_anchor_contact_area(
    ...
    """Return functional anchor contact area.
    
    :param record: The record value.
    :param block_family: The block family value.
    :param raw_contact_area: The raw contact area value.
    :returns: The result produced by the function.
    """
    if str(
        block_family
...
def generate_functional_candidates(
    ...
    """Generate functional candidates.
    
    :param physical_targets: The physical targets value.
    :param structural_segment_ids: Identifiers for the structural segment records.
    :returns: The generated result.
    """
    rows = []
...
def mask_from_selected_candidates(shape, selected_df):
    """Build mask from from selected candidates.
...
def consolidation_sequence_key(sequence):
    """Return consolidation sequence key.
...
def build_consolidated_structural_block(
    ...
    """Build consolidated structural block.
...
def plan_one_segment(
    ...
    :param segment_id: Identifier for the segment.
    :param segment_label: The segment label value.
    :param segment_mask: The segment mask value.
    :param next_block_id: Identifier for the next block.
    :param connector_face_requirements: The connector face requirements value.
    :param inventory_multiplier: The inventory multiplier value.
    :param inventory_scope: The inventory scope value.
    :returns: The result produced by the function.
    """
    segment_id = int(
        segment_id
...
def interface_patch_options(
    ...
    """Return interface patch options.
    
    :param interface_payload: The interface payload value.
    :param normal_a_to_b: The normal a to b value.
    :returns: The result produced by the function.
    """
    axis, sign = (
        FACE_TO_AXIS_SIGN[
...
def connector_record_for_interface(
    ...
    """Return connector record for interface.
...
def generate_required_embedded_connector_candidates(
    ...
    """Generate required embedded connector candidates.
    
    :param required_interfaces_df: DataFrame containing required interfaces records.
    :param interface_payload_by_id: Identifier for the interface payload by.
    :param interface_catalog_queries: The interface catalog queries value.
    :returns: The generated result.
    """
    rows = []
...
def intended_embedded_anchor_overlap(
    ...
    """Return intended embedded anchor overlap.
...
def rotation_candidate_requires_symmetry_centering(row):
    """Return rotation candidate requires symmetry centering.
...
def center_selected_rotation_candidates(
    ...
    """Center selected rotation candidates.
    ...
    audit_rows = []
    effective_center_plane = (
        float(SYMMETRY_CENTER_PLANE)
        if SYMMETRY_CENTER_PLANE is not None
        else float(segment_grid_planner.shape[SYMMETRY_AXIS_INDEX]) / 2.0
    )
    ...
    for row_index, row in centered.iterrows():
        ...
        if not rotation_candidate_requires_symmetry_centering(row):
            audit_rows.append({
                ...
                "symmetry_center_plane": effective_center_plane,
...
def functional_attachment_declaration(attachment_id):
    """Return functional attachment declaration.
    
    :param attachment_id: Identifier for the attachment.
    :returns: The result produced by the function.
    """
...
def required_family_for_attachment(attachment_id):
    """Return required family for attachment.
...
def custom_functional_subassembly_configs():
    """Return custom functional subassembly configs.
...
def custom_functional_subassembly_config_for_target(physical_target_id):
    """Return custom functional subassembly config for target.
    
    :param physical_target_id: Identifier for the physical target.
    :returns: The result produced by the function.
    """
...
def custom_functional_subassembly_enabled(physical_target_id=None):
    """Return whether custom functional subassembly enabled.
...
def catalog_record_for_query(
    ...
    *,
    ...
    """Return catalog record for query.
    
    :param query: The query value.
    :param preferred_size: The preferred size value.
    :returns: The result produced by the function.
    """
    records = [
...
def _default_connector_face_roles(axis="Y"):
    """Return the default connector face roles.
...
def _custom_subassembly_member_templates(config):
    """Return custom subassembly member templates.
...
def create_motion_connected_structural_subassembly(
    ...
    """Create motion connected structural subassembly.
    
    :param config: Configuration values for the operation.
    :param selected_connector_candidate: The selected connector candidate value.
    :param next_block_id: Identifier for the next block.
    :returns: The generated result.
    """
...
def validate_motion_connected_structural_subassembly(
    ...
    """Validate motion connected structural subassembly.
    
    :param config: Configuration values for the operation.
    :param connector_block: The connector block value.
    :param member_blocks: The member blocks value.
...
def functional_segment_group_table(segments_labeled_df):
    """Return functional segment group table.
    
    :param segments_labeled_df: DataFrame containing segments labeled records.
    :returns: The result produced by the function.
    """
    rows = []
...
def candidate_segment_build_axes():
    """Return candidate segment build axes.
...
def restore_block_from_plan_to_world(
    ...
    """Return the restore block from plan to world value.
...
def restore_planning_result_to_world(
    ...
    """Return the restore planning result to world value.
...
def segment_axis_candidate_sort_key(
    ...
    """Return segment axis candidate sort key.
    
    :param candidate: The candidate value.
    :param axis_priority: The axis priority value.
    :returns: The result produced by the function.
    """
    planning_result = (
        candidate[
...
def mirrored_build_axis(
    ...
    """Return mirrored build axis.
    
    :param axis: The axis value.
    :param symmetry_axis_index: The symmetry axis index value.
    :returns: The result produced by the function.
    """
    axis = str(
        axis
...
def interactive_effective_block_opacity(
    ...
    *,
    ...
    """Return interactive effective block opacity.
    
    :param block: Block record used by the operation.
    :param requested_opacity: The requested opacity value.
...
def hover_value_is_present(
    ...
    """Return whether hover value is present.
...
def interactive_build_figure(
    ...
    """Return interactive build figure.
    
    :param blocks: Block records used by the operation.
    :param title: The title value.
    :param include_faces: Whether to include faces.
    :returns: The result produced by the function.
    """
    blocks = list(
        blocks
...
def segmentation_comparison_figure(
    ...
    def visibility_for(mode):
        """Return the visibility for value.
...
def planning_layers_figure(
    ...
    """Return planning layers figure.
    
    :param segment_grid: The segment grid value.
    :param interface_payload: The interface payload value.
    :param connector_candidates: The connector candidates value.
    :param selected_connectors: The selected connectors value.
    :param physical_targets: The physical targets value.
    :param functional_candidates: The functional candidates value.
    :param selected_functional: The selected functional value.
    :returns: The result produced by the function.
    """
    fig = go.Figure()
...
def block_appearance_from_steps(instruction_steps):
    """Return block appearance from steps.
...
def infer_symmetry_center_plane(mask, axis):
    """Return the infer symmetry center plane value.
...
def discover_structural_symmetry_pairs(
    ...
    """Return the discover structural symmetry pairs value.
    ...
    def evaluate_pair(
        ...
        :param semantic_base: The semantic base value.
        :returns: The result produced by the function.
        """
...
def mirrored_structural_rotation(block, axis):
    """Return mirrored structural rotation.
...
def choose_pair_template(pair_row):
    ...
    :param pair_row: The pair row value.
    :returns: The result produced by the function.
    """
...
def mirror_planning_result(
    ...
    """Mirror planning result.
    
    :param template_result: The template result value.
    :param template_segment_id: Identifier for the template segment.
    :param partner_segment_id: Identifier for the partner segment.
    :param partner_label: The partner label value.
    :param partner_mask: The partner mask value.
    :param next_block_id: Identifier for the next block.
    :param center_plane: The center plane value.
    :param partner_connector_face_requirements: The partner connector face requirements value.
    :returns: The result produced by the function.
    """
...
def mirrored_block_signature(
    ...
    """Return mirrored block signature.
...
def symmetric_target_pairs(
    ...
    """Return symmetric target pairs.
    
    :param physical_targets_df: DataFrame containing physical targets records.
    :param segment_grid: The segment grid value.
    :param center_plane: The center plane value.
    :returns: The result produced by the function.
    """
...
def normalized_face_role_signature(face_roles):
    """Return normalized face role signature.
...
def mirrored_face_role_signature(
    ...
    """Return mirrored face role signature.
...
def mirrored_candidate_geometry_signature(
    ...
    """Return mirrored candidate geometry signature.
...
def actual_candidate_geometry_signature(row):
    """Return actual candidate geometry signature.
...
def select_functional_candidates_symmetry_aware(
    ...
    """Select functional candidates symmetry aware.
...
def structural_segment_mirror_map(
    ...
    """Return structural segment mirror map.
    
    :param structural_pairs: The structural pairs value.
    :param segment_grid: The segment grid value.
    :param structural_segment_ids: Identifiers for the structural segment records.
    :param center_plane: The center plane value.
    :returns: The result produced by the function.
...
def pair_structural_interfaces_for_symmetry(
    ...
    """Pair structural interfaces for symmetry.
    
    :param interfaces_df: DataFrame containing interfaces records.
    :param segment_mirror_map: Mapping of segment mirror values.
    :returns: The result produced by the function.
    """
...
def select_connector_candidates_symmetry_aware(
    ...
    """Select connector candidates symmetry aware.
...
def opposite_contact_role(connector_role):
    """Return opposite contact role.
    
    :param connector_role: The connector role value.
    :returns: The result produced by the function.
    """
...
def structural_face_role_possible(face, required_role):
    """Return structural face role possible.
    
    :param face: The face value.
    :param required_role: The required role value.
    :returns: The result produced by the function.
    """
    for record in STRUCTURAL_CATALOG_RECORDS:
        size = tuple(
...
def connector_candidate_receiving_requirements(
    ...
    for side_name, segment_id in [
        ...
        def anchor_sort_key(row):
            """Return the anchor sort key value.
...
def annotate_connector_candidates_with_receiving_faces(
    ...
    """Annotate connector candidates with receiving faces.
...
def explode_selected_connector_face_requirements(
    ...
    """Explode selected connector face requirements.
...
def reservation_normalize_connection_type(value):
    """Return reservation normalize connection type.
...
def reservation_rule_matches(rule, scope, record):
    """Return reservation rule matches.
...
def reservation_explicit_strategy(scope, record):
    """Return reservation explicit strategy.
...
def reservation_structural_interface_decision(interface_row, llm2_decision=None):
    """Return reservation structural interface decision.
    
    :param interface_row: The interface row value.
    :param llm2_decision: The llm2 decision value.
    :returns: The result produced by the function.
    """
...
def reservation_functional_target_decision(target_row, declaration):
    """Return reservation functional target decision.
...
def reservation_rank_anchor_rows(rows, maximum_alternatives):
    """Return reservation rank anchor rows.
    
    :param rows: Row records to process.
    :param maximum_alternatives: The maximum alternatives value.
    :returns: The result produced by the function.
    """
    if not rows:
        return []
...
def reservation_direct_join_requirements(
    ...
    """Return reservation direct join requirements.
    
    :param required_interfaces_df: DataFrame containing required interfaces records.
    :param structural_interface_payload: The structural interface payload value.
    :param llm2_decision_by_interface: The llm2 decision by interface value.
    :returns: The result produced by the function.
    """
    requirement_rows = []
    decision_rows = []
...
def reservation_functional_candidate_receiving_requirements(
    ...
    """Return reservation functional candidate receiving requirements.
    
    :param candidate: The candidate value.
    :param segment_grid: The segment grid value.
    :param reservation_strategy: The reservation strategy value.
    :returns: The result produced by the function.
    """
...
def reservation_annotate_connector_candidates_with_reservations(
    ...
    """Return reservation annotate connector candidates with reservations.
    
    :param candidates_df: DataFrame containing candidates records.
    :param segment_grid: The segment grid value.
    :param interfaces_df: DataFrame containing interfaces records.
    :param llm2_decision_rows: The llm2 decision rows value.
    :returns: The result produced by the function.
    """
...
def reservation_annotate_functional_candidates_with_reservations(
    ...
    """Return reservation annotate functional candidates with reservations.
...
def reservation_selected_functional_requirements(
    ...
    """Return reservation selected functional requirements.
...
def reservation_requirement_mapping(requirements_df):
    """Return reservation requirement mapping.
...
def reservation_reservation_conflicts(requirements_df):
    """Return reservation reservation conflicts.
...
def evaluate_assigned_connector_face_requirements(
    ...
    """Evaluate assigned connector face requirements.
    
    :param current_blocks: The current blocks value.
    :param assignment: The assignment value.
    :param requirements: The requirements value.
    :returns: The result produced by the function.
    """
...
def validate_connector_face_requirements_on_blocks(
    ...
    """Validate connector face requirements on blocks.
...
def progression_lightened_rgb(
    ...
    """Return progression lightened rgb.
...
def clone_block_for_progression_prior(
    ...
    """Clone block for progression prior.
...
def progression_build_axis_label(
    ...
    """Return progression build axis label.
...
def progression_bounds(
    ...
    *,
    ...
    """Return progression bounds.
    
    :param blocks: Block records used by the operation.
    :param fallback_size: The fallback size value.
    :returns: The result produced by the function.
    """
    blocks = list(
        blocks
...
def draw_static_step_block(
    ...
    *,
    ...
    """Draw static step block.
    
    :param axis: The axis value.
    :param block: Block record used by the operation.
    :param alpha: The alpha value.
    :param edgecolor: The edgecolor value.
...
def save_validated_segment_step_images(
    ...
    """Save validated segment step images.
    
    :param result: The result value.
    :param output_dir: Directory where generated artifacts are written.
    :param grid_shape: The grid shape value.
    :returns: The result produced by the function.
    """
    if (
...
def build_complete_timeline(
    ...
    *,
    ...
    :param segment_results: The segment results value.
    :param segment_blocks: The segment blocks value.
    ...
    :param valid_connector_interface_ids: Identifiers for the valid connector interface records.
    :param valid_functional_target_ids: Identifiers for the valid functional target records.
    :returns: The generated result.
    """
    (
        appearance,
...
def detailed_sequence_figure(
    ...
    *,
    ...
    def visibility_for_step(
        ...
        """Return the visibility for step value.
        
        :param step_number: The step number value.
        :returns: The result produced by the function.
        """
        return [
            (
    ...
    def annotation_for_step(
        ...
        """Return the annotation for step value.
    ...
    def annotation_block(
        ...
        """Return the annotation block value.
...
def structuralization_allowed_heights():
    """Return the structuralization allowed heights value.
...
def cell_layer_source_counts(
    ...
    """Return cell layer source counts.
    
    :param segment_grid: The segment grid value.
    :param x: The x value.
    :param y: The y value.
    :param z: The z value.
    :param allowed_segment_ids: Identifiers for the allowed segment records.
    :returns: The result produced by the function.
    """
    values = segment_grid[
        x:x + 2,
...
def optimize_structural_cell_span(
    ...
    """Optimize structural cell span.
    
    :param source_counts_by_layer: The source counts by layer value.
    :param majority_owner_by_layer: The majority owner by layer value.
    :param candidate_segment_ids: Identifiers for the candidate segment records.
...
def apply_confirmed_structural_symmetry_repairs(
    ...
    """Apply confirmed structural symmetry repairs.
...
def structuralize_segment_grid(
    ...
    """Structuralize segment grid.
    
    :param raw_segment_grid: The raw segment grid value.
    :param structural_segment_ids: Identifiers for the structural segment records.
    :param functional_target_segment_ids: Identifiers for the functional target segment records.
    :returns: The result produced by the function.
    """
...
def structuralization_candidate_metrics(result):
    """Return the structuralization candidate metrics value.
...
def structuralization_candidate_objective(metrics, transform):
    """Return the structuralization candidate objective value.
...
def search_structuralization_lattice_offsets(
    ...
    """Search structuralization lattice offsets.
...
def symmetry_exactification_source_score(
    ...
    """Return symmetry exactification source score.
    
    :param candidate_grid: The candidate grid value.
    :param aligned_source_grid: The aligned source grid value.
    :param segment_a: The segment a value.
    :param segment_b: The segment b value.
    :returns: The result produced by the function.
    """
...
def exactify_accepted_structural_symmetry_pairs(
    ...
    """Return the exactify accepted structural symmetry pairs value.
    
    :param structural_grid: The structural grid value.
    :param aligned_source_grid: The aligned source grid value.
    :param accepted_pairs: The accepted pairs value.
    :param center_plane: The center plane value.
    :returns: The result produced by the function.
    """
...
def reference_draw_face_markers(
    ...
    *,
    ...
    """Return reference draw face markers.
    
    :param axis: The axis value.
    :param blocks: Block records used by the operation.
    :param marker_size: The marker size value.
    :param context_blocks: The context blocks value.
    """
    blocks = list(blocks)
    context_blocks = list(
        context_blocks
...
def interactive_face_contact_figure(
    ...
    *,
    ...
    def marker_hover(
        ...
        """Return the marker hover value.
        
        :param row: Row record to process.
        :param face_type: The face type value.
        :returns: The result produced by the function.
        """
        segment_id = row[
            "segment_id"
...
def assembly_block_traces_with_opacity(
    ...
    *,
    ...
    """Return assembly block traces with opacity.
    
    :param block: Block record used by the operation.
    :param opacity: The opacity value.
    :param showlegend: The showlegend value.
    :param name_suffix: The name suffix value.
    :param preview: The preview value.
    :returns: The result produced by the function.
    """
    traces = block_traces(
...
def assembly_player_step_annotation(row):
    """Return assembly player step annotation.
...
def assembly_visible_assembly_player(
    ...
    *,
    ...
    """Return assembly visible assembly player.
    
    :param assembly_steps_df: DataFrame containing assembly steps records.
    :param segment_blocks_by_id: Identifier for the segment blocks by.
    :param connector_blocks: The connector blocks value.
    :param interface_payload: The interface payload value.
    :param title: The title value.
...
def interface_connector_rule(interface_row):
    """Return interface connector rule.
...
def persist_segment_result(
    ...
    :param segment_id: Identifier for the segment.
    :param label: The label value.
    :param packable_mask: The packable mask value.
    """
    segment_id = int(segment_id)
