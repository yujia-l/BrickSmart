"""Validated row/column build orchestration entry points.

This module resolves contracts, inventory profiles, run paths, and planner
inputs before delegating to the catalog-driven row/column build engine.
"""
...
def _write_inventory_infeasibility_artifacts(
    *, output_dir: Path, outcome: dict[str, Any], inventory_profile_path: str | Path | None
    ...
    """Write inventory infeasibility artifacts.
    ...
    :param inventory_profile_path: Path to the inventory profile file.
    :type inventory_profile_path: str | Path | None
    """
...
def _confirmation_rows(path: Path | None) -> list[dict[str, Any]]:
    """Return the confirmation rows value.
...
def _write_semantic_preservation(
    *, context_path: Path, output_dir: Path, confirmations_path: Path | None
    ...
    :type confirmations_path: Path | None
    :returns: The result produced by the function.
    :rtype: dict[str, Any]
    """
...
def run_model_build(
    *,
    ...
    :type project_root: str | Path | None
    :param inventory_profile_path: Path to the inventory profile file.
    :type inventory_profile_path: str | Path | None
    :param clean_output: The clean output value.
    :type clean_output: bool
    :param check: The check value.
    :type check: bool
    :param allow_unverified_contract: Whether to allow unverified contract.
    :type allow_unverified_contract: bool
    :param model_source_override: The model source override value.
    :type model_source_override: str | dict[str, Any] | None
    :param run_id: Identifier for the run.
    :type run_id: str | None
    :param timeout_seconds: Optional wall-clock limit for the planner subprocess.
    :type timeout_seconds: float | None
    :returns: The result produced by the function.
    :rtype: RowColumnRunResult
    """
    ...
    command = [sys.executable, "-m", "bricksmart.row_column_engine"]
    timed_out = False
    with log_path.open("w", encoding="utf-8") as log:
        try:
            ...
            log.write(
                "\nBrickSmart planner stopped after reaching the configured "
                f"{float(timeout_seconds or 0):g}-second wall-clock limit.\n"
            )
