...
Run:
    python -m app.services.ingest                 # load all bundles from GCS with Vertex embeddings
...
def ingest_from_gcs(settings=settings, embed=True, dest=None, db=None):
    ...
    if dest:
        ...
        count = repository.load_knowledge_from_gcs(
            settings=settings,
            embed=embed,
            db=db,
        )
...
def _main():
    ...
    parser.add_argument("--no-embed", action="store_true",
                        help="load rows WITHOUT Vertex embeddings (schema/plumbing smoke test)")
    parser.add_argument("--keep", default="",
                        help="keep the downloaded GCS mirror in this dir (default: a temp dir)")
    parser.add_argument(
        "--backfill-only",
        action="store_true",
        help="resume missing Vertex embeddings in rows already loaded into Cloud SQL",
    )
    parser.add_argument(
        "--max-embeddings",
        type=int,
        default=100,
        help="maximum rows to embed during a resumable backfill run",
    )
    parser.add_argument(
        "--bundle-name",
        default="",
        help="limit embedding backfill to one processed bundle name",
    )
    args = parser.parse_args()

    embed = not args.no_embed
    if embed and not settings.GCP_PROJECT_ID:
        raise RuntimeError("GCP_PROJECT_ID is required for Vertex embeddings")
    if args.backfill_only:
        count = repository.backfill_embeddings(
            settings=settings,
            limit=args.max_embeddings,
            bundle_name=(args.bundle_name or None),
        )
        print(f"done: backfilled {count} embedding(s)")
        return
