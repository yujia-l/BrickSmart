...
def load_bundle_payload(session, bundle, nodes, rules_json=None, settings=None, embed=True):
    ...
    existing = {
        row.node_id: row
        for row in session.query(PdfNode).filter_by(bundle_id=b["bundle_id"]).all()
    }
    vectors_by_node: dict[str, list[float]] = {}
    if embed:
        pending = []
        for node in nodes:
            ...
            unchanged = bool(
                current
                and current.text == node.get("text")
                and current.embedding is not None
                and current.embedding_model == settings.EMBED_MODEL
                and current.embedding_dimensions == settings.EMBED_DIM
            )
            if not unchanged:
                pending.append(node)
        if pending:
            vectors = embed_texts([node.get("text", "") for node in pending], settings)
            vectors_by_node = {
                node["node_id"]: vector for node, vector in zip(pending, vectors)
            }
        _log.info(
            "embedding %d/%d changed or missing node(s) for %s",
            len(pending),
            len(nodes),
            b["bundle_name"],
        )
    ...
    for n in nodes:
        ...
        values = dict(
            ...
            visual_embedding=n.get("visual_embedding"),
            visual_embedding_model=n.get("visual_embedding_model"),
            updated_at=_now())
        if n["node_id"] in vectors_by_node:
            values.update(
                embedding=vectors_by_node[n["node_id"]],
                embedding_model=settings.EMBED_MODEL,
                embedding_dimensions=settings.EMBED_DIM,
            )
        current = existing.get(n["node_id"])
        if current is None:
            session.add(
                PdfNode(
    ...
    if rules_json:
        existing_rules = {
            row.rule_id: row
            for row in session.query(StandardRule)
            .filter_by(bundle_name=b["bundle_name"])
            .all()
        }
        for rule in rules_json.get("rules", []):
            rule_id = rule.get("rule_id")
            values = dict(
                policy_scope=rules_json.get("policy_scope"),
                framework=rule.get("framework"),
                grade_band=rule.get("grade_band"),
                lab=rule.get("lab"),
                unit=rule.get("unit"),
            ...
            current = existing_rules.get(rule_id)
            if current is None:
                session.add(
                    StandardRule(
...
def load_knowledge_from_gcs(settings=None, embed=True, db=None, bundle_names=None):
    ...
    the embeddings call goes out to Vertex AI. Each bundle is loaded and committed individually.
    """
    settings = settings or _SETTINGS
...
def backfill_embeddings(
    ...
    """Resume missing or stale text embeddings without reloading GCS artifacts.

    Each batch is committed independently. A quota interruption can therefore be
    retried later without losing completed vectors or repeating finished work.
    """
    settings = settings or _SETTINGS
    db = db or DBUtil(settings)
    session = db.get_session()
    ...
    try:
        while remaining:
            ...
            query = session.query(PdfNode).filter(
                (PdfNode.embedding.is_(None))
                | (PdfNode.embedding_model.is_(None))
                | (PdfNode.embedding_model != settings.EMBED_MODEL)
                | (PdfNode.embedding_dimensions.is_(None))
                | (PdfNode.embedding_dimensions != settings.EMBED_DIM)
            )
            if bundle_name:
                query = query.filter(PdfNode.bundle_name == bundle_name)
            rows = query.order_by(PdfNode.id).limit(size).all()
            if not rows:
                break
            vectors = embed_texts([row.text or " " for row in rows], settings)
            for row, vector in zip(rows, vectors):
                row.embedding = vector
                row.embedding_model = settings.EMBED_MODEL
                row.embedding_dimensions = settings.EMBED_DIM
                row.updated_at = _now()
            session.commit()
            completed += len(rows)
            remaining -= len(rows)
            _log.info("embedding backfill committed %d node(s)", completed)
    except Exception:
        session.rollback()
        raise
    finally:
...
_NODE_COLS = ("n.node_id, n.type, n.text, n.doc_kind, n.page_id, n.page_no, n.visual_role, "
              ...
              "n.lesson, n.image_type, n.educational_purpose, n.ocr_text, n.image_uri")
...
def retrieve_from_db(query, settings=None, filters=None, k=30, seed_k=5, hybrid=True, db=None):
    ...
    vector_where = list(where) + ["n.embedding IS NOT NULL"]
    vector_wsql = "WHERE " + " AND ".join(vector_where)
    vec_sql = (f"SELECT {_NODE_COLS}, "
               f"(n.embedding::halfvec({dim}) <=> (:qv)::vector({dim})::halfvec({dim})) AS distance "
               f"FROM pdf_node n {vector_wsql} "
