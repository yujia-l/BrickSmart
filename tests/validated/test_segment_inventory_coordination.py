...
def test_all_source_segments_receive_a_build_phase(catalog_csv: Path):
    """Test that all source segments receive a build phase.
    
    :param catalog_csv: The catalog csv value.
    :type catalog_csv: Path
    """
    result, _ = _run(catalog_csv)
...
def test_each_segment_appears_in_one_contiguous_step_range(catalog_csv: Path):
    """Test that each segment appears in one contiguous step range.
...
def test_global_reservations_match_final_recount(catalog_csv: Path):
    """Test that global reservations match final recount.
    
    :param catalog_csv: The catalog csv value.
    :type catalog_csv: Path
    """
    result, ledger = _run(catalog_csv)
...
def test_global_alternatives_enable_limited_backtracking(catalog_csv: Path):
    """Test that global alternatives enable limited backtracking.
...
def test_v5_outputs_use_catalog_colors_and_catalog_lineage(
    tmp_path: Path, catalog_csv: Path
):
    """Test that v5 outputs use catalog colors and catalog lineage.
    ...
    assert "block_definitions.csv" not in html
