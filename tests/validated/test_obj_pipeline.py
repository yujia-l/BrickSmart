...
def _run(catalog_csv: Path, teacher_budget=None):
    """Run run.
    
    :param catalog_csv: The catalog csv value.
    :type catalog_csv: Path
    :param teacher_budget: The teacher budget value.
...
def test_base_obj_integration_harness_passes_with_csv_catalog(catalog_csv: Path):
    """Test that base obj integration harness passes with csv catalog.
    ...
    assert result.catalog_summary["source_path"].endswith("block_definitions.csv")
...
def test_zero_structural_budget_fails_without_exceeding_inventory(
    ...
    """Test that zero structural budget fails without exceeding inventory.
    ...
    result, ledger = _run(catalog_csv, budget)
...
def test_obj_outputs_include_catalog_lineage_and_preview(
    ...
    """Test that obj outputs include catalog lineage and preview.
