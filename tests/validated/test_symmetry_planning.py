...
def bird_symmetry_build(module_catalog_csv: Path):
    ...
    return run_obj_build(
        ...
        catalog_path=module_catalog_csv,
...
def test_detects_bird_bilateral_plane_and_source_symmetry(bird_symmetry_build):
    """Test that detects bird bilateral plane and source symmetry.
    
    :param bird_symmetry_build: The bird symmetry build value.
    """
...
def test_all_selected_blocks_have_exact_mirrors(bird_symmetry_build):
    """Test that all selected blocks have exact mirrors.
...
def test_mirrored_source_segments_receive_equal_block_counts(bird_symmetry_build):
    """Test that mirrored source segments receive equal block counts.
...
def test_inventory_is_committed_as_atomic_symmetry_groups(bird_symmetry_build):
    """Test that inventory is committed as atomic symmetry groups.
...
def test_symmetry_is_a_hard_final_gate(bird_symmetry_build):
    """Test that symmetry is a hard final gate.
