...
def _read_xlsx_sheet_records(path: str | Path, sheet_name: str) -> list[dict[str, Any]]:
    path = Path(path)
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if "xl/sharedStrings.xml" in archive.namelist():
            ...
            for string_item in shared_root.findall(f"{{{main_namespace}}}si"):
                shared_strings.append(
                    "".join(
                        node.text or ""
                        for node in string_item.iter(f"{{{main_namespace}}}t")
                    )
                )
        ...
        worksheet_path = None
        sheets_node = workbook_root.find(f"{{{main_namespace}}}sheets")
        if sheets_node is None:
            raise KeyError(f"No worksheets found in {path}")
        ...
        if worksheet_path is None:
            raise KeyError(f"Worksheet {sheet_name!r} was not found in {path}")

        worksheet_root = ET.fromstring(archive.read(worksheet_path))
        row_maps: list[dict[int, Any]] = []
        maximum_column = -1
        for row in worksheet_root.iter(f"{{{main_namespace}}}row"):
            ...
            for cell in row.findall(f"{{{main_namespace}}}c"):
                ...
                if cell_type == "inlineStr" and inline_node is not None:
                    value = "".join(
                        node.text or ""
                        for node in inline_node.iter(f"{{{main_namespace}}}t")
                    )
    ...
    if not row_maps:
        return []
    matrix = [
        [row_map.get(column_index) for column_index in range(maximum_column + 1)]
...
def _catalog_colors(catalog_path: str | Path, sheet_name: str = "Block Definitions") -> dict[str, str]:
    records = _read_xlsx_sheet_records(catalog_path, sheet_name)
    colors = {
        str(record.get("block_family") or "").strip(): str(record.get("color") or "").strip()
        for record in records
        if record.get("block_family")
    }
    if not colors:
        raise CatalogConfigurationError(
            f"No block_family/color rows found in {catalog_path} sheet {sheet_name!r}"
        )
    return colors
