...
def test_obj_endpoint_uses_original_workbook():
    response = client.post("/api/obj/plan", json={"model_uri": "model://bird-base"})
    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] in {"PASS", "FAIL_GEOMETRY_VALIDATION"}
    assert payload["catalog"]["source_path"].endswith("block_definitions.xlsx")
    assert payload["catalog"]["sheets_read"] == ["Block Definitions"]
