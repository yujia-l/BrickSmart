...
def test_original_xlsx_is_the_only_block_catalog() -> None:
    catalog_dir = ROOT / "block_catalog"
    assert (catalog_dir / "block_definitions.xlsx").is_file()
    assert list(catalog_dir.glob("block_definitions.csv")) == []
...
def test_runtime_context_uses_xlsx_and_separate_inventory() -> None:
    context = json.loads(
        (ROOT / "model_registry/contracts/bird-standard-kit/versions/validated-1/task_context.json")
        .read_text(encoding="utf-8")
    )
    assert context["paths"]["catalog_xlsx"].endswith("block_definitions.xlsx")
